...
| 2026-04-10 | Phase 10-A (feat/trust-score): Added graduated trust score system (0-1000) | Trust tiers: PROBATION (0-199), STANDARD (200-499), TRUSTED (500-799), VETERAN (800-1000). Payout staging time based on tier. Dynamic referral caps and lock period reduction for high-trust users. Monthly recalculation cron. All existing fraud rules now adjust trust score on flag insertion. VIP accounts start at 500. | migration 20260410000001, packages/api/src/trustScore.ts, fraudRules.ts, payout/request/route.ts, recurring-payouts/route.ts, trust-score-recalc/route.ts, auth/callback/route.ts, vercel.json |
| 2026-04-10 | Phase 10-B (feat/ip-classification): Added IP infrastructure classification using hardcoded CIDR ranges | Datacenter IPs get -50 trust on signup. Clusters of 5+ signups from same datacenter /24 in 7 days trigger CRITICAL R19 flag. Replaces VPN detection stub from PR 2-D. VIP accounts exempt from datacenter penalties. MaxMind API deferred to 500+ users. | migration 20260410000002, packages/api/src/ipClassification.ts, lockPeriod.ts, auth/callback/route.ts, fraud-scan/route.ts, SECURITY.md |
| 2026-04-10 | Phase 10-C (feat/referral-source): Added referral source tracking via Referer header | Sources classified as GREEN (social platforms), YELLOW (unknown), RED (from admin blocklist). R17 flags referrers whose referees come from red-flagged sources (3+ standard, 10+ VIP). Admin API for managing source blocklist. /ref/[code] route handler captures Referer and passes through signup flow to referral row. | migration 20260410000003, packages/api/src/sourceClassification.ts, ref/[code]/route.ts, signup/page.tsx, signup/actions.ts, auth/callback/route.ts, fraud-scan/route.ts, api/admin/source-blocklist/* |
| 2026-04-11 | Phase 10-E (feat/graph-topology): Referral graph topology analysis with 5 detection patterns (R16) | Runs every 6 hours. 5 patterns: star clusters, bipartite swaps, disconnected cliques, fan-out convergence, gen2 velocity. BFS-based cycle detection with depth limit 8 (chosen over recursive SQL CTE for safety). Components processed with hard limits (500 nodes, 200 components) to prevent cron timeout. payout_destination convergence deferred to after Phase 5. graph_analysis_results table is NOT append-only (admins resolve patterns). | migration 20260411000003, packages/api/src/graphAnalysis.ts, cron/graph-analysis/route.ts, vercel.json, SECURITY.md |
| 2026-04-11 | Phase 10-F (feat/honeypot): Honeypot & canary fraud detection system | Honeypot accounts have referral codes leaked on forums — signups via those codes get CRITICAL fraud flag + -200 trust. Canary accounts are seeded as fake referees — anyone referring a canary is flagged. Deactivation sets boolean to false (not delete) to preserve referral history. @honeypot.internal and @canary.internal email domains are internal-only fakes. VIP exception downgrades severity to INFO. Confirm-referrals cron skips honeypot/canary referrals entirely. | migration 20260411000002, auth/callback/route.ts, confirm-referrals/route.ts, api/admin/honeypot/*, api/admin/canary/*, admin/honeypot/*, admin/users/*, SECURITY.md |
| 2026-04-11 | Phase 10-D (feat/signup-telemetry): Signup funnel anomaly detection | Client-side telemetry collects form fill timing and input corrections. link_click_at passed via URL param from /ref/[code]. Trust penalties: fast signup (<10s) -40, fast form fill (<5s) -30, no corrections -5 (reduced from -15 to avoid penalizing accurate typists and password manager users). Scroll tracking removed — signup page fits in viewport so scroll_events===0 is the normal case. Soft signals only — never blocks signup. | migration 20260411000001, signup/page.tsx, signup/actions.ts, ref/[code]/route.ts, auth/callback/route.ts |
